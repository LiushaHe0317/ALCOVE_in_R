...
p = colMeans(matrix(result2[,1,drop = FALSE],
                    nrow = 50, ncol = 10))
Err = colMeans(matrix(result2[,2,drop = FALSE],
                      nrow = 25, ncol = 20))
att = result[[3]]
# layout 2x2 graphics
par(mfrow = c(2,2))
...
# plot results
plot(c(1:20), Err, 
     xlab = 'No. of Block', ylab = 'Error',
     main = 'Gradient Descent', pch = 18)
abline(lm(Err ~ c(1:20)))

persp(c(1:25),c(1:25),result1,
      xlab = 'Dimension X', ylab = 'Dimension Y',
      zlab = 'Activation',main = 'Hidden layer')

barplot(att, xlab = 'Dimension', ylab = 'Attention Strength',
        main = 'Dimensional Attention', names.arg = c('X','Y'),
        ylim = c(0,1))
